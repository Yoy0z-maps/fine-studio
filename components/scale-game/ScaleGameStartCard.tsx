...
import { ScaleGameMode, ScaleDifficulty } from "@/types/scale";

interface ScaleGameStartCardProps {
  gameMode: ScaleGameMode;
  difficulty: ScaleDifficulty;
  onModeChange: (mode: ScaleGameMode) => void;
  onDifficultyChange: (level: ScaleDifficulty) => void;
  onStart: () => void;
}

export default function ScaleGameStartCard({
  gameMode,
  difficulty,
  onModeChange,
  onDifficultyChange,
  onStart,
}: ScaleGameStartCardProps) {
    ...
    <View style={[styles.container, { backgroundColor: colors.surface }]}>
      ...
      <View style={styles.modeRow}>
        ...
        >
          ...
          >
            {t("scaleGame.playMode")}
          </AppText>
        </Pressable>
      </View>

      <AppText style={[styles.description, { color: colors.textSecondary }]}>
        {gameMode === "identify" ? t("scaleGame.identifyDesc") : t("scaleGame.playDesc")}
      </AppText>

      {/* 난이도 선택 */}
      <View style={styles.difficultyRow}>
        {([1, 2, 3, 4, 5] as ScaleDifficulty[]).map((level) => (
          <Pressable
            key={level}
            style={[
